angular
  ...
  .factory('delegateStorage', function ($rootScope, $q) {
    function s4() {
    ...
    function DelegatedStorageArea(connectionPromise, type) {
      ...
      $q.when(connectionPromise).then(function (connection) {
        ...
        connection.port.onMessage.addListener(function (message) {
          if (
            message.from.tab != connection.tabId ||
            message.from.app != connection.appId
          ) {
            return;
          }
      ...
      ['get', 'set', 'remove', 'clear', 'getBytesInUse'].forEach(function (
        val,
      ) {
