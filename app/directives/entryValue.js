angular.module('storageExplorer').directive('entryValue', function ($compile) {
  ...
  var objectTemplate =
    '<span class="bracket">{</span><span class="object">{{displayedValue}}</span><span class="bracket">}</span>';
  var arrayTemplate =
    '<span class="bracket">[</span>{{displayedValue}}<span class="bracket">]</span>';
  ...
  function string(value) {
    return stringTemplate.replace(
      '{{displayedValue}}',
      checkWidth(escape(JSON.stringify(value))),
    );
  }
