...
PortManager.prototype.onPortDisconnected = function (
  app,
  tab,
  disconnectedPort,
) {
...
PortManager.prototype.trackUiPort = function (app, tab, port) {
  ...
  port.onMessage.addListener(function (message) {
    console.log(
      'Received message from ui port,  app:tab ' + app + ':' + tab,
      message,
    );
    ...
    if (targetPort) {
      ...
      console.error(
        'Target port not found for  app id ' + app + ' and tab id ' + tab,
      );
    }
  });
};
...
PortManager.prototype.trackTargetPort = function (app, tab, port) {
  ...
  if (!this.getUiPort(app, tab)) {
    port.disconnect();
    throw new Error(
      'Target port cannot be tracked before ui port exist. Id ' +
        app +
        ':' +
        tab,
    );
  ...
  port.onMessage.addListener(function (message) {
    ...
    if (uiPort) {
      ...
      self.onPortDisconnected(app, tab, port);
      console.error(
        "Can't find ui port for appId " + app + ' and tab Id' + tab,
      );
    }
  });
};
...
function initializeExtension(runtime, extension, $document, tabs) {
  ...
  extension.onConnect.addListener(function (port) {
    if (port.name.indexOf('for_tab_') === 0) {
      ...
      tabs.executeScript(
        tabId,
        { file: 'app/chrome/htmlStorageHook.js' },
        function (e, p) {
  ...
  extension.onConnectExternal.addListener(function (externalPort) {
    ...
    var tabId = externalPort.sender.tab
      ? externalPort.sender.tab.id
      : undefined;
